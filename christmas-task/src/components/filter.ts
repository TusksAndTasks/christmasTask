     ...
     createFilteredData(data: mainData[]): mainData[]{
        ...
        if(colorCheck){
            if(fullFilteredData.length === 0){
                colorButtons.forEach((button: Element): void =>{
                    if((button as HTMLElement).classList.contains('active')){
                        filteredData = data.filter((infoCard: mainData) => infoCard.color === (button as HTMLElement).dataset.value);
                        fullFilteredData.push(...filteredData);
                    }
                })
            } else {
                colorButtons.forEach((button: Element): void =>{
                    if((button as HTMLElement).classList.contains('active')){
                    filteredData = fullFilteredData.filter((infoCard: mainData) => infoCard.color === (button as HTMLElement).dataset.value);
        ...
        if(sizeCheck){
            if(fullFilteredData.length === 0){
                ...
                sizeButtons.forEach((button: Element): void =>{
                    if((button as HTMLElement).classList.contains('active')){
                    filteredData = fullFilteredData.filter((infoCard: mainData) => infoCard.size === (button as HTMLElement).dataset.value);
